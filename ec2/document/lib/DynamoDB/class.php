...
class DynamoDB
{
    static private function getClient(): Aws\DynamoDb\DynamoDbClient
    {
        ...
        if ($client === null) {
            ...
            $client = (new Aws\Sdk([
                "version" => "latest",
                "region"  => Secret::get("/dynamodb/env.json")["region"],
            ]))->createDynamoDb();
        }
    ...
    {
        // normalize data
        if (isset($data["TableName"])) $data["TableName"] = Secret::get("/dynamodb/env.json")["prefix"] . $data["TableName"];
        if (isset($data["ExpressionAttributeValues"])) $data["ExpressionAttributeValues"] = array_map(fn ($value) => self::encode($value), $data["ExpressionAttributeValues"]);

        if (isset($data["Item"])) {
            if ($data["Item"]["sort"] ?? null) $data["Item"]["sort"] = (string)$data["Item"]["sort"];
            $data["Item"] = array_map(fn ($value) => self::encode($value), $data["Item"]);
        }

        if (isset($data["Key"])) $data["Key"] = array_map(fn ($value) => self::encode($value), $data["Key"]);
    ...
    static public function query(array $data): array
    {
        ...
        return $response && isset($response["Items"]) && is_array($response["Items"]) ? array_map(function (array $item) {
            return array_map(fn ($value) => self::decode($value), $item);
        }, $response["Items"]) : [];
    }
    ...
    {
        $response = self::request(__FUNCTION__, $data);
        return $response && isset($response["Item"]) && $response["Item"] && is_array($response["Item"]) ? array_map(fn ($value) => self::decode($value), $response["Item"]) : null;
    }
